...
// NOVA LISTA PARA ARMAZENAR AS COLETAS
final List<Map<String, dynamic>> coletas = [];
...
String generateToken(String username) {
  ...
  final jwt = JWT({
    'username': username,
    'exp':
        DateTime.now().add(Duration(hours: 24)).millisecondsSinceEpoch ~/ 1000,
...
void main() async {
  ...
  final handler = Pipeline()
      ...
      .addMiddleware(
        DebugMiddleware.performanceMonitor(slowRequestThreshold: 500),
      )
  ...
  router.post('/api/cadastro', (Request request) async {
    ...
    try {
      ...
      logger.error('Erro no processamento do cadastro', {
        'error': e.toString(),
      }, stackTrace);
  ...
  router.get('/api/produtores', (Request request) {
    logger.debug('Listagem de produtores solicitada', {
  ...
  // Inicio do novo endpoint

  // Endpoint para cadastrar uma nova coleta
  router.post('/api/coletas', (Request request) async {
    logger.info('Nova solicitação de registro de coleta');

    try {
      final body = await request.readAsString();
      final data = jsonDecode(body);

      logger.debug('Dados da coleta recebidos', data);
      ...
      final novaColeta = {
        ...
        'alizarol': data['alizarol'],
        'status': 'Coletado',
      };

      coletas.add(novaColeta);

      logger.info('Coleta registrada com sucesso', {
        'id': novaColeta['id'],
        'produtor': novaColeta['nomeProdutor'],
      ...
      logger.error('Erro ao registrar coleta', {
        'error': e.toString(),
      }, stackTrace);
      ...
      return Response(
        ...
        body: jsonEncode({
          'success': false,
          'message': 'Erro ao registrar coleta: $e',
        }),
        headers: {'Content-Type': 'application/json'},
      );
    }
  ...
  router.get('/api/coletas', (Request request) {
    logger.debug('Listagem de coletas solicitada', {'total': coletas.length});

    return Response.ok(
      jsonEncode({'success': true, 'data': coletas}),
      headers: {'Content-Type': 'application/json'},
    );
  });
  ...
  router.get('/api/relatorios', (Request request) {
    ...
    final relatorios = {
      ...
      'volumeTotal': historico.fold(
        0.0,
        (sum, item) => sum + (item['quantidade'] as num),
      ),
  ...
  router.get('/api/debug/logs', (Request request) async {
    final lines =
        int.tryParse(request.url.queryParameters['lines'] ?? '50') ?? 50;
  ...
  router.get('/api/debug/errors', (Request request) async {
    final lines =
        int.tryParse(request.url.queryParameters['lines'] ?? '20') ?? 20;
  ...
  router.post('/api/debug/rotate-logs', (Request request) async {
    ...
    try {
      ...
      return Response.ok(
        jsonEncode({
          'success': true,
          'message': 'Logs rotacionados com sucesso',
        }),
        headers: {'Content-Type': 'application/json'},
      );
    } catch (e, stackTrace) {
      logger.error('Erro na rotação de logs', {
        'error': e.toString(),
      }, stackTrace);
      ...
      return Response(
        ...
        body: jsonEncode({
          'success': false,
          'message': 'Erro na rotação de logs',
        }),
        headers: {'Content-Type': 'application/json'},
      );
    }
  ...
  try {
    ...
    print('   POST /api/coletas'); // NOVO
    print('   GET  /api/coletas'); // NOVO
    ...
    logger.critical('Falha ao iniciar servidor', {
      'error': e.toString(),
    }, stackTrace);
    exit(1);
  }
