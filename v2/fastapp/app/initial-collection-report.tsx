...
import { ChevronLeft, Calendar, Thermometer, TestTube, User, Save } from 'lucide-react-native';
...
export default function InitialCollectionReportScreen() {
  ...
  const validateForm = () => {
    ...
    if (!formData.tankNumber.trim()) newErrors.tankNumber = 'Tank number is required';
    if (!formData.producerCode.trim()) newErrors.producerCode = 'Producer code is required';
    if (!formData.producerName.trim()) newErrors.producerName = 'Producer name is required';
    if (!formData.temperature.trim()) newErrors.temperature = 'Temperature is required';
    else if (isNaN(Number(formData.temperature))) newErrors.temperature = 'Temperature must be a number';
    if (!formData.alizarolTest.trim()) newErrors.alizarolTest = 'Alizarol test result is required';
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      ...
      Alert.alert(
        'Success',
        'Initial Collection Report saved successfully!',
        [
          {
            text: 'OK',
            onPress: () => router.back(),
      ...
      <View className="flex-1">
        ...
        <View className="bg-blue-500 pt-12 pb-6 px-4 shadow-sm">
          <View className="flex-row items-center mb-4">
            ...
            <Text className="text-white text-xl font-bold">Initial Collection Report</Text>
          </View>
          <Text className="text-blue-100 text-sm ml-12">Fill in the collection details</Text>
        </View>
        ...
        <ScrollView className="flex-1 px-4 py-6">
          ...
          {/* Tank Information */}
          <View className="bg-white rounded-xl p-4 mb-4 shadow-sm">
            <Text className="text-gray-700 font-bold mb-3">Tank Information</Text>
            ...
            <View className="mb-3">
              ...
              />
              {errors.tankNumber ? <Text className="text-red-500 text-xs mt-1">{errors.tankNumber}</Text> : null}
            </View>
          </View>
          
          {/* Producer Information */}
          <View className="bg-white rounded-xl p-4 mb-4 shadow-sm">
            <Text className="text-gray-700 font-bold mb-3">Producer Information</Text>
            ...
            <View className="mb-3">
              ...
              />
              {errors.producerCode ? <Text className="text-red-500 text-xs mt-1">{errors.producerCode}</Text> : null}
            </View>
            ...
            <View className="mb-3">
              ...
              />
              {errors.producerName ? <Text className="text-red-500 text-xs mt-1">{errors.producerName}</Text> : null}
            </View>
          </View>
          ...
          <View className="bg-white rounded-xl p-4 mb-4 shadow-sm">
            ...
            <View>
              <Text className="text-gray-600 text-sm mb-1">Temperature (°C)</Text>
              <TextInput
          ...
          <View className="bg-white rounded-xl p-4 mb-4 shadow-sm">
            ...
            <View>
              ...
              />
              {errors.alizarolTest ? <Text className="text-red-500 text-xs mt-1">{errors.alizarolTest}</Text> : null}
            </View>
          </View>
          ...
          <View className="bg-white rounded-xl p-4 mb-6 shadow-sm">
            ...
            <View>
              ...
              <TextInput
                ...
                onChangeText={(value) => handleChange('responsiblePerson', value)}
                placeholder="Enter responsible person name"
                className={`border rounded-lg p-3 ${errors.responsiblePerson ? 'border-red-500' : 'border-gray-300'}`}
              />
              {errors.responsiblePerson ? <Text className="text-red-500 text-xs mt-1">{errors.responsiblePerson}</Text> : null}
            </View>
          </View>
        </ScrollView>
